...
class IssueFilter extends Component {
  ...
  renderButton() {
    if (this.props.filter.imageUrl) {
      return (
        <img
          src={this.props.filter.imageUrl}
          alt={this.props.filter.publisher}
        />
      );
    } else {
  ...
  render() {
    return (
      <button
        className={'pub-logo ' + (this.props.active ? '' : 'inactive')}
        key={this.props.filter.publisher}
        data-publisher={this.props.filter.publisher}
